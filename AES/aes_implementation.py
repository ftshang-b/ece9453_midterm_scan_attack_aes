...
class AesFromScratch:
    ...
    def initTables(self):
        ...
        self.RCON = ["01", "02", "04", "08", "10", "20", "40", "80", "1b",
                     "36"]
    ...
    def keySchedule(self, key, round):
        ...
        roundKey[0] = self.xor(words[0],
                               self.SboxMatch(words[3], round)).zfill(8)
    ...
    def shiftRows(self, state):
        ...
        # No Shift
        newState[0], newState[4], newState[8], newState[12] = state[0], state[
            4], state[8], state[12]

        # First shift. Shift by one
        newState[1], newState[5], newState[9], newState[13] = state[5], state[
            9], state[13], state[1]

        # Second shift. Shift by two
        newState[2], newState[6], newState[10], newState[14] = state[10], \
                                                               state[14], \
                                                               state[2], state[
                                                                   6]

        # Third shift. Shift by three
        newState[3], newState[7], newState[11], newState[15] = state[15], \
                                                               state[3], state[
                                                                   7], state[
                                                                   11]
        # print(newState)
